...
@stop
@php
    if (!empty(old('bu_Latitude'))){
        $lat =  old('bu_Latitude');
    }elseif ($bu->bu_Latitude != ''){
        $lat = $bu->bu_Latitude;
    }else{
          $lat = '30.06277690073326';
    }

if (!empty(old('bu_longitude'))){
    $lang =  old('bu_longitude');
}elseif ($bu->bu_longitude != ''){
    $lang = $bu->bu_longitude;
}else{
...
@section('content')
    <div class='container'>
        <div class='span8 main'>
            ...
            <hr>
            <h1>تعديل العقار :  {{ $bu->bu_name }}</h1>
            <hr>
            {!! Form::model($bu,['method'=>'PATCH', 'action'=>['BuController@userBuildingUpdate', $bu->id], 'files'=>true]) !!}
            <div class="group">
            ...
            <img src="{{ checkImage($bu->image, 'buImages/') }}" alt="" width="100" height="100">
